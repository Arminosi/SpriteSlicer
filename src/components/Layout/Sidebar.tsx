...
export const Sidebar: React.FC = () => {
  const { settings, updateSettings, file, setFile, t, resetCells } = useSlicer();
    ...
    <aside className="w-80 h-full border-r border-border bg-surface/50 flex flex-col shrink-0 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <Tooltip key={mode} content={t(`sort.${mode.replace('-', '')}` as any)}>
                  <button
      ...
      <div className="mt-auto border-t border-border p-4 bg-surface/30">
        <div className="flex flex-col gap-2">
          ...
          <a 
            href="https://github.com/Arminosi/SpriteSlicer" 
            target="_blank" 
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 px-3 py-2 rounded-md bg-surface hover:bg-surface-hover border border-border hover:border-primary/50 transition-all text-xs text-gray-400 hover:text-primary group"
          >
